...
public class LaserBoiRecipes extends SerializableRecipe {
	...
	public void registerDefaults() {
		makeRecipe(new ComparableStack(ModItems.billet_silicon, 1), new ComparableStack(OreDictManager.DictFrame.fromOne(ModItems.circuit, ItemCircuit.EnumCircuitType.SILICON)));
		makeRecipe(new ComparableStack(ModItems.billet_gaas, 1), new ComparableStack(OreDictManager.DictFrame.fromOne(ModItems.circuit, ItemCircuit.EnumCircuitType.GAAS)));
		makeRecipe(new ComparableStack(ModItems.ingot_dineutronium), new ComparableStack(ModItems.dust_tiny));
	}
	...
	public void readRecipe(JsonElement recipe) {
		...
		makeRecipe(new ComparableStack(input), new ComparableStack(output));
	}
	...
	public static void makeRecipe(ComparableStack in, ComparableStack out){
		if(out == null) {
			MainRegistry.logger.error("Laser Engraver recipe returned null!");
			return;
		}
		engraverRecipe recipe = new engraverRecipe(in, out);
	...
	public static class engraverRecipe {
		...
		public engraverRecipe(ComparableStack input, ComparableStack output) {
			this.input = input;
			this.output = output;
		}
	}
