...
public class TileEntityMachineLaserBoi extends TileEntityMachineBase implements IEnergyReceiverMK2, IGUIProvider {
	...
	public boolean canProcess() {
		if(slots[0] != null && slots[2] != null && power > 0){ //Base checks. Nested ifs are nasty but i'm prioritizing readability.
			if(slots[1] == null){
				...
				return itemList.containsKey(slots[0].getItem()) && crystalList.containsKey(slots[2].getItem()) && slots[1].isItemEqual(itemList.get(slots[0].getItem())) && slots[1].stackSize < slots[1].getMaxStackSize();
			}
		}
		return false;
	}
	...
	public void process() {
		...
		if (process >= processSpeed/crystalList.get(slots[2].getItem())) {
			...
			} else if (slots[1] != null && slots[1].isItemEqual(itemList.get(slots[0].getItem()))) {
				slots[1].stackSize++;
			}
